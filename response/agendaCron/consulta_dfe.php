...
$_POST['consultSefaz'] = true;

#Validar tipo da requisição
$consultaSefaz  = ( $_POST['consultSefaz'] ? true : false );
$efetuaDownload = false;//( $_POST['dfeDownload']  ? true : true );
    ...
    if ($consultaSefaz) {
        ...
        $diffTIme = gmdate('H:i:s', strtotime(date("H:i:s")) - strtotime(date('H:i:s')));
          ...
          if ( $lastConsult->RecordCount() <= 0 ){
            $bd->Execute("INSERT INTO t_dfe_service ( data_consulta, hora_consulta, user_consulta, lastnsu ) VALUES ('{$dataConsulta}', '{$horaConsulta}', '64', '{$lasNsu->fields['lastnsu']}');");
            $lastConsult    = $bd->Execute($sql = "SELECT hora_consulta, id_t_dfe_service, lastnsu FROM t_dfe_service WHERE data_consulta = '{$dataConsulta}' ORDER BY 1 DESC LIMIT 1;");
            $statusConsulta = true;
          }
        ...
        if (!$timeOut) {
            ...
            $ultNSU = 1;//$lastConsult->fields['lastnsu'];
            $maxNSU = $ultNSU;
            $loopLimit = 50;
            $iCount = 0;
            ...
            while ($ultNSU <= $maxNSU) {
                ...
                try {
                    if ($statusConsulta) {
                        ...
                        $bd->Execute("INSERT INTO t_dfe_service ( data_consulta, hora_consulta, user_consulta, lastnsu ) VALUES ('{$dataConsulta}', '{$horaConsulta}', '1', '{$ultNSU}');");                        
                ...
                print "<pre> "; print_r($_SESSION["dfe_retorno"]);
//                
//                exit;
...
//                    exit;
                    ...
                    $node = $dom->getElementsByTagName('retDistDFeInt')->item(0);                   $tpAmb = $node->getElementsByTagName('tpAmb')->item(0)->nodeValue;
                    $verAplic = $node->getElementsByTagName('verAplic')->item(0)->nodeValue;        $cStat = $node->getElementsByTagName('cStat')->item(0)->nodeValue;
                    $xMotivo = $node->getElementsByTagName('xMotivo')->item(0)->nodeValue;          $dhResp = $node->getElementsByTagName('dhResp')->item(0)->nodeValue;
                    $ultNSU = $node->getElementsByTagName('ultNSU')->item(0)->nodeValue;            $maxNSU = $node->getElementsByTagName('maxNSU')->item(0)->nodeValue;
...
//                        print"<pre>";
//                        print_r($doc->getAttribute('schema'));
//                        exit;
...
//                        exit;
                        ...
                        if ($tipo == 'resNFe') {
                            ...
                            $chNFe = $std->chNFe;                               $cnpj = $std->CNPJ;
                            $xNome = $std->xNome;                               $ie = $std->IE;
                            $tpNF = $std->tpNF;                                 $vNF = $std->vNF;
                            $dhRecbto = $std->dhRecbto;                         $nProt = $std->nProt;
                            $cSitNfe = $std->cSitNFe;                           $digVal = $std->digVal;
                            $arquivo = $chNFe . "--{$tipo}.xml";                $xSchema = $tipo;

                            //Dados que são vazios pra essa condição
                            $cUF = "";              $cNF = "";              $serie = "";            $mod = "";
                            $dhEmi = "";            $verAplic = "";         $cStat = "";            $xMotivo = "";
                            $idEvento = "";         $tpEvento = "";         $xEvento = "";          $nSeqEvento = "";       
                        ...
                        // -- Definindo o tipo procNF
                            ...
                            #Organizando os dados que serão gravados na base.
                            $chNFe = $std->protNFe->infProt->chNFe;             $cnpj = $std->NFe->infNFe->emit->CNPJ;
                            $xNome = $std->NFe->infNFe->emit->xNome;            $ie = $std->NFe->infNFe->emit->IE;
                            $tpNF = $std->NFe->infNFe->ide->tpNF;               $vNF = $std->NFe->infNFe->total->ICMSTot->vNF;
                            $dhRecbto = $std->protNFe->infProt->dhRecbto;       $nProt = $std->protNFe->infProt->nProt;
                            $digVal = $std->protNFe->infProt->digVal;           $arquivo = $chNFe . "--{$tipo}.xml";
                            $cUF = $std->NFe->infNFe->ide->cUF;                 $cNF = $std->NFe->infNFe->ide->cNF;
                            $serie = $std->NFe->infNFe->ide->serie;             $mod = $std->NFe->infNFe->ide->mod;
                            $dhEmi = $std->NFe->infNFe->ide->dhEmi;             $verAplic = $std->protNFe->infProt->verAplic;
                            $cStat = $std->protNFe->infProt->cStat;             $xMotivo = $std->protNFe->infProt->xMotivo;
                            $xSchema = $tipo;                                   $idEvento = $std->NFe->infNFe->attributes->Id;

                            //Dados que são vazios pra essa condição
                            $cSitNfe = "";          $tpEvento = "";             $xEvento = "";
                            $nSeqEvento = "";       $dhRegEvento = "";          $dhEvento = "";
                        ...
                        // -- Definindo o tipo procEv
                            ...
                            #Organizando os dados que serão gravados na base.
                            $chNFe = $std->evento->infEvento->chNFe;            $cnpj = $std->evento->infEvento->CNPJ;
                            $nProt = $std->retEvento->infEvento->nProt;         $arquivo = $chNFe . "--{$tipo}.xml";
                            $xSchema = $tipo;                                   $idEvento = $std->evento->infEvento->attributes->Id;
                            $xMotivo = $std->retEvento->infEvento->xMotivo;     $cStat = $std->retEvento->infEvento->cStat;
                            $verAplic = $std->retEvento->infEvento->verAplic;   $tpEvento = $std->retEvento->infEvento->tpEvento;
                            $xEvento = $std->retEvento->infEvento->xEvento;     $nSeqEvento = $std->retEvento->infEvento->nSeqEvento;
                            $dhEvento = $std->retEvento->infEvento->dhEvento;   $dhRegEvento = $std->retEvento->infEvento->dhRegEvento;                            

                            //Dados que são vazios pra essa condição
                            $xNome = "";            $ie = "";               $tpNF = "";             $vNF = "0";
                            $dhRecbto = "";         $cSitNfe = "";          $digVal = "";
                        ...
                        // -- Definindo o tipo resEve
                            ...
                            #Organizando os dados que serão gravados na base.
                            $chNFe = $std->chNFe;                               $cnpj = $std->CNPJ;
                            $dhEvento = $std->dhEvento;                         $tpEvento = $std->tpEvento;
                            $nSeqEvento = $std->nSeqEvento;                     $xEvento = $std->xEvento;
                            $dhRecbto = $std->dhRecbto;                         $nProt = $std->nProt;
                            $arquivo = $chNFe . "--{$tipo}.xml";                $xSchema = $tipo;
                            ...
                            $cUF = "";              $cNF = "";              $serie = "";            $mod = "";
                            $dhEmi = "";            $verAplic = "";         $cStat = "";            $xMotivo = "";
                            $idEvento = "";         $dhRegEvento = "";      $digVal = "";           $cSitNfe = "";
                            $xNome = "";            $ie = "";               $tpNF = "";             $vNF = "0";
                        ...
                        if ($dadosValid->RecordCount() <= 0) {
                            $id_t_dfe_service = $lastConsult->fields['id_t_dfe_service'];
                            //Após a leitura do Documento vamos gravar no banco de dados
                            $bd->Execute($sql = "
                                INSERT INTO t_dfe_service_docs(
...
//        requestEmail("nfe");
        ...
        $dataXML = $bd->Execute($sql = "SELECT chnfe, substring(dhrecbto,1,4) AS ano, substring(dhrecbto, 6,2) AS mes, status 
...
if ($_POST['tipoBusca'] == "unica") {
    ...
    while (( $statusConsulta != "CIENCIA_OPERACAO" && $statusConsulta != "CIENCIA_OPERACAO_DOWNLOAD") || $cont <= 1) {
        ...
        $verificaCiencia = $bd->Execute($sql = "SELECT chave_acesso_consultada 
...
if ($timeOut) {
    $retornoValue = "TIME_OUT";
} else {
    $retornoValue = "SUCESSO";
}
print json_encode([
    "status" => $retornoValue
]);
